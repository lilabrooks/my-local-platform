...
# Widens the window further, and is kept under RELAY_DELIVERY_TIMEOUT so
# attempts fail on their own terms rather than timing out.
...
# A latency at or above the delivery timeout makes every attempt time out, so
# /hooks/ok would never succeed and there would be no first delivery to
# duplicate. Read off the container rather than restated, so it cannot drift.
...
if [ "$timeout_ms" -gt 0 ] && [ "$LATENCY_MS" -ge "$timeout_ms" ]; then
  fail "LATENCY_MS=${LATENCY_MS} is not below RELAY_DELIVERY_TIMEOUT=${timeout_spec}.
    Every attempt would time out, /hooks/ok would never succeed, and there
    would be no first delivery to duplicate. Pick a latency under ${timeout_ms}ms."
fi
note "delivery timeout is ${timeout_spec}; ${LATENCY_MS}ms is under it"

say "clearing the sink and slowing it to ${LATENCY_MS}ms"
...
note "delivered as $first_id"
note "the offset stays uncommitted until EVERY subscriber for the record is finished"
...
# Measured here: that policy does NOT bring the container back from a SIGKILL --
# it sat Exited (137) indefinitely. It restarts the exit-1 case it was added for
# (a fatal broker fetch, see the comment on this service in
# local/docker-compose.yml), which is a different signal.
#
...
waited=0
while [ "$(delivered_ids | grep -c "^${first_id}$")" -lt 2 ]; do
  waited=$((waited + 1))
  if [ "$waited" -ge 90 ]; then
    note "ids seen on /hooks/ok for this run: $(delivered_ids | tr '\n' ' ')"
    fail "$first_id was delivered once and never redelivered.
...
note "webhook-id $first_id delivered $count times"
...
pass "a crash between delivery and commit redelivered the SAME webhook-id ($count times)"
